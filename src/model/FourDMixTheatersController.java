...
public class FourDMixTheatersController {
    private int price = 0;
    private User user;
    private Movie movie;
    private Round round;
    ...
    @FXML
    ImageView imageMovie,image4DChair,imageScreen;

    @FXML
    Label textTitle,textPrice;

    @FXML
    Button A1, A2, A3, A4, A5, A6, A7, A8, B1, B2, B3, B4, B5, B6, B7, B8, C1, C2, C3, C4, C5, C6, C7, C8;


    @FXML
    ImageView image4DNormalChair;
    ...
    public void initialize(){
        Platform.runLater(new Runnable() {
            ...
            public void run() {
                ...
                Button[] chairs = {A1, A2, A3, A4, A5, A6, A7, A8, B1, B2, B3, B4, B5, B6, B7, B8, C1, C2, C3, C4, C5, C6, C7, C8};
                for(Button e : chairs){
                    if(e.getId().charAt(0) == 'A'){
                        if(isVisit(e.getId())){
                            if(isUser(e.getId())){
                                e.setStyle("-fx-background-image: url('/images/c4m.png')");
                            }else{
                                e.setStyle("-fx-background-image: url('/images/c4c.png')");
                                e.setDisable(true);
                            }
                        }else{
                            e.setStyle("-fx-background-image: url('/images/c4.png')");
                        }
                    }else {
                        if(isVisit(e.getId())){
                            if(isUser(e.getId())){
                ...
                textTitle.setText(movie.getName()+" - 4D - "+round.getTime());
                try {
                    image4DChair.setImage(new Image(getClass().getResource("/images/c4.png").toURI().toString()));
                    image4DNormalChair.setImage(new Image(getClass().getResource("/images/c3.png").toURI().toString()));
    ...
    public void handleChooseHoneyAction(ActionEvent event) throws IOException {
        Button b = (Button) event.getSource();
        if(b.getStyle().equals("-fx-background-image: url('/images/c4m.png')")){
            int dialogResult = JOptionPane.showConfirmDialog (null, "Cancel reservation");
            if(dialogResult == JOptionPane.YES_OPTION){
                remove(b.getId());
            }
        }else if(b.getStyle().equals("-fx-background-image: url('/images/c4.png')")){
            b.setStyle("-fx-background-image: url('/images/c4c.png')");
            price += 400;
            chairOrder.add(b);
        }else{
            b.setStyle("-fx-background-image: url('/images/c4.png')");
            price -= 400;
            if(price < 0){
                price = 0;
            }
